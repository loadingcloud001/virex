...
async def ui_camera_detail(
    ...
    playback = build_playback_urls(cam.mtx_path)
    ...
    return _render(
        ...
        webrtc_url=playback.webrtc_url,
...
async def ui_dashboard(
    ...
    so the dashboard takes 5 round-trips to Postgres instead of 7.
    """
    ...
    hb_cutoff = datetime.now(timezone.utc) - timedelta(minutes=5)
    ...
    node_row = (
        await db.execute(
            select(
                func.count().label("total"),
                func.sum(
                    case((Node.last_heartbeat_at >= hb_cutoff, 1), else_=0)
                ).label("online"),
    ...
    # Query 5: cameras — for the camera snapshot panel.
    cam_rows = (
        await db.execute(
            select(Camera)
            .where(Camera.tenant_id == tenant_id)
    ...
    # Use a typed schema (UserOut) for `user` so the template is
    # decoupled from the SQLAlchemy User model. The tenant_slug needs
    # a separate fetch because TenantMiddleware resolves from Host
    # header, not from the user row.
    tenant_row = (
        await db.execute(
