...
All endpoints are tenant-scoped via ``user.tenant_id`` (the session user's
tenant, cross-validated by ``require_admin_session``). The
``/api/events/table`` endpoint returns raw HTML fragments for HTMX
consumption; the JSON ``/api/events`` endpoint returns the same data
as a flat list.
...
async def list_events_json(
    ...
    items, _total = await _query_events(
        db, user.tenant_id, camera_id, window, limit, offset
    )
    return items
...
async def list_events_table(
        ...
        cam_rows = (
            await db.execute(
                select(Camera.id, Camera.name, Camera.mtx_path).where(
                    Camera.tenant_id == user.tenant_id, Camera.id.in_(cam_ids)
                )
            )
        ).all()
        cam_name_map = {
    ...
    # the filtered result is empty. ``total`` is always an int from a
    # COUNT query, so f-string interpolation here is injection-safe —
    # do NOT wrap in Markup because Markup + str concatenation escapes
    # the template-rendered HTML on the right-hand side.
    oob_count = f'<span id="events-count-value" hx-swap-oob="true">{int(total)}</span>'
...
async def get_event(
    ...
    event = (
        await db.execute(
            select(Event).where(
                Event.id == event_id, Event.tenant_id == user.tenant_id
            )
        )
    ).scalar_one_or_none()
    if event is None:
